...
<body class="container-fluid pt-2">
    ...
    <header class="">
        <header class="text-center">
            ...
            <p class="lead">Visualize abaixo a lista de usuários cadastrados no sistema.</p>
        </header>
    </header>
    <main class="table-responsive small mt-2 min-vh-75">
        <table id="tabela" class="table table-hover table-sm align-middle pb-2">
            ...
            <?php if($action == "result"): ?>
                <?php foreach($resultadoPaginado as $usuario): ?>
                <tr class="p-2">
                    <td class="p-2"><a class="link-dark link-underline link-underline-opacity-0" href="/user/edit?id=<?php echo $usuario->getId();?>"><?php echo $usuario->getNome();?></td>
                    <td class="p-2"><a class="link-dark link-underline link-underline-opacity-0" href="/user/edit?id=<?php echo $usuario->getId();?>"><?php echo $usuario->getEmail();?></td>
                    <td class="p-2"><a class="link-dark link-underline link-underline-opacity-0" href="/user/edit?id=<?php echo $usuario->getId();?>"><?php echo ucfirst(strtolower($usuario->getPerfil()));?></td>
                    <td class="p-2"><a class="link-dark link-underline link-underline-opacity-0" href="/user/edit?id=<?php echo $usuario->getId();?>"><?php if($usuario->getAtivo()) echo "Sim"; else echo "Não";?></td>
                    <td class="p-2">
                        <a class="link-dark link-underline link-underline-opacity-0" href="/user/edit?id=<?php echo $usuario->getId();?>">
                            <div class="form-check d-flex justify-content-center">
    ...
    <nav class="page navigation d-flex justify-content-center align-items-center" aria-label="Page navigation">
        ...
        <?php foreach($navigation as $navItem): ?>
            <li class="page-item<?php if(!$navItem['active']) echo " disabled"; ?>">
                <a class="page-link link-dark" href="/user/list?page=<?php echo $navItem['page']; ?>">
